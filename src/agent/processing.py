...
def process_new_email_notification(email: str, start_history_id: int):
    ...
    try:
        ...
        for message in new_messages:
            ...
            initial_state = { "user_id": user.id, "email_content": full_email_content }
            ...
            if triage_result and triage_result.action_required:
                ...
                # Get the subject from the reliable Triage Result
                email_summary = triage_result.summary
                # Convert the tool output dict to a clean string for display
                action_result_str = json.dumps(tool_outputs[0] if tool_outputs else {"status": "No output."})

                summary = (
                    f"Aura took action on an email about: '{email_summary}'.\n"
                    f"Action Result: {action_result_str}"
                )

                notification_payload = { "type": "proactive_agent_action", "data": {"summary": summary} }
